...
if [ -e /etc/redhat-release ]; then
	...
	#elif grep "5." /etc/redhat-release >> /dev/null; then
	 #OS="RHEL5"
	 #BLANKCKL="$STIGDIR/blank_rhel5.ckl"
	fi
...
# $STIGDIR/$OS/prestage.sh
 ...
 if [ "$(uname -p)" = "sparc" ]; then
  OS="SOL11"
  cp $STIGDIR/$OS/SPARC/* $STIGDIR/$OS
  chmod +x $STIGDIR/$OS/*
  BLANKCKL="$STIGDIR/CKL_Templates/blank_sol11_sparc.ckl"
 #else in for Solaris 11 x86
 else 
  OS="SOL11"
  cp $STIGDIR/$OS/X86/* $STIGDIR/$OS
  chmod +x $STIGDIR/$OS/*
  BLANKCKL="$STIGDIR/CKL_Templates/blank_sol11_x86.ckl"
 fi
#elif Solaris 10
else
 exit
