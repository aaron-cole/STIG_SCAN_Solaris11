...
RuleID="SV-61081r1_rule"
STIGID="SOL-11.1-050380"
...
if [ -e /etc/issue ] && [ "$(cat /etc/issue)" = "$banner" ]; then
 if [ -e /etc/issue ] && [ "$(cat /etc/motd)" = "$banner" ]; then
  echo "/etc/issue and /etc/motd are configured correctly" >> $Results
  echo "Pass" >> $Results
 else
  echo "FAIL - /etc/motd" >> $Results
  echo "Fail" >> $Results
 fi
else
