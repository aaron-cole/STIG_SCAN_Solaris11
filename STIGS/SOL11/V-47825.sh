...
RuleID="SV-60701r1_rule"
STIGID="SOL-11.1-010340"
...
if [ "$(zonename)" != "global" ]; then
 ...
 if  pfexec auditconfig -getflags | grep active |sed s/'active user default audit flags ='// | grep fa >> $Results; then
  if pfexec auditconfig -getnaflags | grep active |sed s/'active user default audit flags ='// | grep fa >> $Results; then
